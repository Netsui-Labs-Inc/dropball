<div>
    @if($edit)
        <div class="form-group row">
            ...
            <div class="col-md-10">
                <input type="text" name="rate" class="form-control" value="{{ number_format($hubCommissionRate, 4) }}%" disabled/>
            </div>
        </div>
    @endif
    ...
    @else
        @if($edit)
            <div class="d-none">
                <input type="hidden" name="whole_number_rate" class="form-control" value="{{ $defaultWholeNumber }}" />
                <input type="hidden" name="decimal_number_rate" class="form-control" value="{{ $defaultDecimalNumber }}"/>
            </div>
        @endif
    @endif
